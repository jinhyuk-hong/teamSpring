...
insert into like_history(user_id, user_email, book_title, book_author, book_isbn, 
		...
		values
		("lady1580", "lady.1580@daum.net" , "안녕", "임다혜", "9788949121277", "https://image.aladin.co.kr/product/750/4/cover500/8949121271_1.jpg", 
		"2010-07-29", "비룡소");
        
select count(*) as 'total_count', book_title, book_isbn, book_cover
					, book_pubDate, book_publisher, book_author
from loan_history 
where loan_date between concat('2023', '-','01', '-01') and concat('2023', '-', '01', '-31' ) 
group by book_isbn 
...
CREATE TABLE `buy_history` (
  `buy_no` int(11) NOT NULL AUTO_INCREMENT, -- 대출 도서 번호
  `book_price` int(255) not null, -- 구매 가격
  `user_id` varchar(20) NOT NULL, -- 구매 회원 아이디
  `user_email` varchar(40) not NULL, -- 구매 회원 이메일
  `book_title` varchar(100) NOT NULL, -- 구매 도서 제목
  `book_author` varchar(200) NOT NULL, -- 구매 도서 저자
  `book_isbn` varchar(20) NOT NULL, -- 구매 도서 ISBN
  `book_cover` varchar(2000) DEFAULT NULL, -- 구매 도서 표지 주소
  `book_pubdate` varchar(20) NOT NULL, -- 구매 도서 출간일
  `book_publisher` varchar(50) NOT NULL, -- 구매 도서 출판사
  `buy_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 구매 일자
  `return_date` timestamp NULL DEFAULT NULL, -- 반납 일자
  `return_period` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 도서 배달 기한
  `return_status` tinyint(1) NOT NULL DEFAULT '0', -- 도서 반납 상태
  PRIMARY KEY (`buy_no`),
  KEY `buy_history_FK` (`user_id`),
  CONSTRAINT `buy_history_FK` FOREIGN KEY (`user_id`) REFERENCES `member` (`user_id`)
